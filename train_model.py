...
def seg_sentences(sentence):
    ...
    sentence = re.sub(
        ...
        "",
        sentence)
    sentence_seged = ' '.join(jieba.cut(sentence.strip()))
    return sentence_seged


df['文本分词'] = df['通话内容'].apply(seg_sentences)
df["文本分词"] = df["文本分词"].apply(lambda x: x.split(" "))
...
index, weight, default, pad, bos, eos = get_text_embedding(
    r"C:\Users\15581\Documents\上海电信\20_04\datas_211_0407.txt_vectors.txt", "<pad>", "<bos>", "<eos>")
...
# (self, vocab, embed_size, num_channels, pooling_ints, dropout_rate,dense)
model = Textcnn(vocab_size, embed_size, 100, 50, 0.2, [512, 238])
